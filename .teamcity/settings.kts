...
project {
    ...
    params {
        ...
        text("github_repository_name", "MergeTool", label = "The github repository name. Used to connect to it in VCS Roots.", description = "This is the repository slug on github. So for example `MergeTool` or `MinecraftForge`. It is interpolated into the global VCS Roots.", display = ParameterDisplay.HIDDEN, allowEmpty = false)
        text("env.PUBLISHED_JAVA_ARTIFACT_ID", "mergetool", label = "Published artifact id", description = "The maven coordinate artifact id that has been published by this build. Can not be empty.", allowEmpty = false)
    ...
    features {
        githubIssues {
            id = "MergeTool__IssueTracker"
            displayName = "neoforged/MergeTool"
            repositoryURL = "https://github.com/neoforged/MergeTool"
        }
    }
}
...
object BuildSecondaryBranches : BuildType({
    ...
    id("MergeTool__BuildSecondaryBranches")
